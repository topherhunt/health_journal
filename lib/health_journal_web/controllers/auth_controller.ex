defmodule HealthJournalWeb.AuthController do
  ...
  def signup_submit(conn, %{"user" => user_params}) do
    case Data.insert_user(user_params, :owner) do
      {:ok, user} ->
        ...
        HealthJournal.Emails.confirm_address(user, user.email) |> HealthJournal.Mailer.send()
  ...
  def login_submit(conn, %{"user" => %{"email" => email, "password" => password}}) do
    ...
    cond do
      ...
      true ->
        ...
        conn
        |> HealthJournalWeb.AuthPlugs.login!(user)
  ...
  def logout(conn, _params) do
    conn
    |> HealthJournalWeb.AuthPlugs.logout!()
    |> redirect(to: Routes.page_path(conn, :index))
  end
  ...
  def request_password_reset_submit(conn, %{"user" => %{"email" => email}}) do
    if user = Repo.get_by(User, email: email) do
      HealthJournal.Emails.reset_password(user) |> HealthJournal.Mailer.send()
    end
